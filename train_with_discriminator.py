...
class Solver(object):
    ...
    def train(self):
        ...
        keys = ["G/loss_id", "G/loss_id_psnt", "G/loss_cd", "D/loss_d"]
        ...
        for i in range(self.num_iters):
            ...
            # discrminator loss
            real_pro = self.D(x_real)
            fake_pro = self.D(x_identic_psnt.squeeze())
            d_loss_d = self.discriminator_loss(real_pro, fake_pro)

            # Backward and optimize.
            g_loss = g_loss_id + g_loss_id_psnt + self.lambda_cd * g_loss_cd + d_loss_d
            self.reset_grad()
            g_loss.backward()
...
if __name__ == "__main__":
    ...
    wandb.run.name = args.model_name + "_DC"
    wandb.run.save()
